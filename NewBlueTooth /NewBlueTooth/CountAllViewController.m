...
/*
    ...
    NSString *date = [self getDate:senddate];
    
    self.lastDate = [NSUserDefaults standardUserDefaults];
    NSString *s = [self.lastDate objectForKey:@"lastdate"];
    
    if (s.length <= 0) {
        [self.lastDate setObject:date forKey:@"lastdate"];
    }
    
    else {
        if ([s isEqualToString:date]) {
            NSLog(@"今天和昨天相同,保存数据但不写入数据库");
        
            for (int i = 0; i < 8; i++) {
                StepData *data = [[StepData alloc]init];
                data.week = [self getWeek:senddate];
                data.date = date;
                data.steps = 12000;
//               [StepDataDB addData:data];
            ...
            NSUserDefaults *lastAllSteps = [NSUserDefaults standardUserDefaults];
            
            NSString *lastSteps = [lastAllSteps objectForKey:@"lastAllSteps"];//昨日之前总步数
            int yesterdaySteps = [self.steps.lastObject intValue] -  [lastSteps intValue];//昨日步数
            
            [lastAllSteps setObject:self.steps.lastObject forKey:@"lastAllSteps"];//将新的总步数记录
            
            NSLog(@"新建一个对象,写入数据库");
            StepData *data = [[StepData alloc]init];
            data.week = [self getWeek:senddate];
            data.date = date;
            data.steps = yesterdaySteps;
...
//        all.text = [NSString stringWithFormat:@"%d",allSteps];
...
//        all.text = self.steps.lastObject;//总步数显示
    }];
}
